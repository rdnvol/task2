class FacetMoreLess extends HTMLElement {
  constructor() {
    ...
    this.filterItems = JSON.parse(this.querySelector('script#filter-items').innerHTML);
    this.itemsToFilter = null;

    if (!Object.values(this.filterItems)?.length === 0 || this.filterItems?.filter_values?.length <= 5) {
      this.querySelector('[data-id="more"]').style.display = 'none';

      return;
    }

    this.addEventListener('click', function (e) {
      if (e.target.dataset.id === 'more') {
        this.clickCount += 1;
      } else {
        this.clickCount -= 1;
      }
      ...
      if (this.clickCount >= 1) {
        this.querySelector('[data-id="less"]').style.display = 'inline';
      }

      if (e.target.dataset.id === 'less') {
        if (this.clickCount * 5 + 5 < this.itemsToFilter.length) {
          this.querySelector('[data-id="more"]').style.display = 'inline';
        }

        this.items = this.parentElement.querySelectorAll(`.py-3.custom-input.custom-input--${this.queryVar}-with-text`);
        ...
        if (this.clickCount === 0) {
          this.querySelector('[data-id="less"]').style.display = 'none';
        }
      } else {
        if (this.clickCount * 5 + 5 >= this.itemsToFilter.length) {
          this.querySelector('[data-id="more"]').style.display = 'none';
        ...
        this.insertAfterEl.insertAdjacentHTML(
          'afterend',
          this.itemsToFilter.slice(this.clickCount * 5, this.clickCount * 5 + 5).join('')
        );
      }
    });
  }
...
class FacetFiltersForm extends HTMLElement {
  constructor() {
    ...
    const filterSelectors = document.querySelectorAll('#filter-sortby, #filter-sortby-2');
  ...
  static renderSectionFromFetch(url, event) {
    fetch(url)
      ...
      .then((responseText) => {
        ...
        FacetFiltersForm.triggerUpdateQuickViewEvent();
      });
  }
  ...
  static renderProductGridContainer(html) {
    ...
    const rerenderedPageEvent = new Event('collectionPageRerender');

    document.dispatchEvent(rerenderedPageEvent);
  }

  static renderPagination(html) {
    const paginationWrapper = document.getElementById('pagination-container');

    if (!paginationWrapper) return false;

    paginationWrapper.innerHTML = new DOMParser()
      .parseFromString(html, 'text/html')
  ...
  static triggerUpdateQuickViewEvent() {
    const updateQuickViewEvent = new CustomEvent('updateQuickView');

    document.dispatchEvent(updateQuickViewEvent);
  }
